...
import { getLifespanTicks } from '../sim/Dinos';
...
function render(world: World): void {
  ...
  const symbol = dino.sex === 'male' ? '♂' : '♀';
  panel = makePanel(`${symbol} ${dino.name}`);
...
function populate(panel: HTMLElement, world: World, dino: Dino): void {
  ...
  addRow(stats, 'Genome', `${Math.round(dino.dnaPercentAtHatch)}%`);
  addRow(stats, 'Lifespan', formatLifespan(getLifespanTicks(dino, species)));
}
...
function formatLifespan(ticks: number): string {
  ...
  const years = Math.floor(ticks / yearTicks);
  const months = Math.floor((ticks - years * yearTicks) / monthTicks);
